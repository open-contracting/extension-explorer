...
def _get_types(value, sources, extension_version, lang, n=1, field=None):
    ...
    if "codelist" in codelist_schema and types == ["string"]:
        ...
        if open_codelist is True:
            return [
                ngettext(
                    'string from open <a href="%(url)s">%(codelist)s</a> codelist',
                    'strings from open <a href="%(url)s">%(codelist)s</a> codelist',
                    n,
                    **variables,
                )
            ]
        if open_codelist is False:
            return [
                ngettext(
                    'string from closed <a href="%(url)s">%(codelist)s</a> codelist',
                    'strings from closed <a href="%(url)s">%(codelist)s</a> codelist',
                    n,
                    **variables,
                )
            ]
        return [
            ngettext(
                'string from <a href="%(url)s">%(codelist)s</a> codelist',
                'strings from <a href="%(url)s">%(codelist)s</a> codelist',
                n,
                **variables,
            )
        ]
...
def _codelist_url(basename, extension_version, lang):
    ...
    if basename in extension_version["codelists"]:
        url = url_for(
            "extension_codelists",
            lang=lang,
            identifier=extension_version["id"],
